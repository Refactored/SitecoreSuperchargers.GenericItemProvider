...
namespace SitecoreSuperchargers.GenericItemProvider.Superchargers
{
    public static class ItemExtension
    {
        ...
        public static IEnumerable<Item> RemoveIdFromMultilist(this Item item, string fieldId, string valueToRemove,
        ...
        public static IEnumerable<Item> AddIdToMultilist(this Item hostItem, string fieldName, string newId,
                                                         bool checkSecurity = false)
        {
        ...
        /// <summary>
        /// Populates entity class with content from a Sitecore item
        /// </summary>
        /// <param name="item">The Sitecore Item that is the source data</param>
        /// <param name="entity">The IEntity object to populate</param>
        /// <returns>A populated IEntity objects</returns>
        public static IEntity ToEntity(this Item item, IEntity entity)
        {
            Assert.IsNotNull(item, "item cannot be null");
            ...
            {
                var fieldId = Helpers.FieldMappingHelper.GetFieldId(property);
                if (fieldId.IsNullOrEmpty()) continue;

                var fieldMapping = Helpers.FieldMappingHelper.GetFieldMapping(property);
                var itemFieldValue = item.Fields[fieldMapping.FieldId].GetValue(true);
                var mapping = fieldMapping as IConvertibleAttribute;
                if (mapping != null) itemFieldValue = mapping.Unconvert(itemFieldValue);

                if (!String.IsNullOrEmpty(itemFieldValue))
                {
                    property.SetValue(entity, System.Convert.ChangeType(itemFieldValue, property.PropertyType), null);
                }
            }
